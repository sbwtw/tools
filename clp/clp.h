...
namespace nm
{
  ...
  public:
    ...
    Clp(int argc, char* argv[]) noexcept : map_{}, cmd_{}, c_{}
    {
      for(int i = 1; i < argc; ++i)
        cmd_.emplace_back(argv[i]);
    }

    Clp(const std::string& path) : map_{}, cmd_{}, none_{}, c_{}, msg_{}
    {
      errno = 0;
    ...
    Clp(Clp&& rhs) noexcept
        : map_{std::move(rhs.map_)}, cmd_{std::move(rhs.cmd_)}, none_{std::move(rhs.none_)}, c_{std::move(rhs.c_)},
    ...
    bool ok() const noexcept { return msg_.empty(); }
    ...
    {
      static_assert(strict_same<bool, T>::value || strict_same<int, T>::value || strict_same<long, T>::value ||
                        strict_same<long long, T>::value || strict_same<unsigned int, T>::value ||
                        strict_same<unsigned long, T>::value || strict_same<unsigned long long, T>::value ||
                        strict_same<float, T>::value || strict_same<double, T>::value ||
                        strict_same<long double, T>::value || strict_same<std::string, T>::value,
                    "\ntypename must be:\n"
                    "boolean type,\n"
    ...
    const std::set<std::string>& none() const noexcept { return none_; }

    bool contain(const std::string& key) const noexcept { return c_.find(key) != c_.end(); }
